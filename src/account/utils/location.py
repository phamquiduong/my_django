...
def refresh_vn_location_data(json_path: Path = settings.VN_LOCATION_RESOURCE_DIR):
    ...
    logger.info('Inserting Province data...')
    Province.objects.bulk_create(provinces)
    logger.info('Inserted Province data successfully')

    logger.info('Inserting Ward data...')
    Ward.objects.bulk_create(wards)
    logger.info('Inserted Ward data successfully')
